#!/usr/bin/env python3
"""Experiment v15a: Bigger decoder to break the 3% RMSE wall.

Hypothesis: The decoder is the bottleneck, not VQ quality.
Evidence: Both collapsed VQ (v9g, 3.13%) and healthy VQ (v14b, 2.95%) hit ~3%.
          8 RVQ levels barely improved over 3 levels despite 34-47% utilization.

Changes from v14b-A (2.95% RMSE baseline):
  - d_model: 128 -> 256  (2x decoder width)
  - n_decoder_layers: 4 -> 6  (50% more depth)
  - d_ff: 512 -> 1024  (2x FFN width)
  - n_encoder_layers: 4 -> 4  (keep encoder same)
  - batch_size: 12 + accum_steps=6 for VRAM safety
  - 80 epoch probe (not full 200)

Same VQ config as v14b-A: vq_dim=16, 128 codes, 8 RVQ levels, 16 patches.
Same storage: 129 bytes/window = 39.7x raw.
"""
...
def train_hvqvae(encoder, decoder, train_loader, val_loader, config, device,
                 epochs=80, save_name="v15a", accum_steps=6):
    encoder.to(device)
    decoder.to(device)
...
def main():
    ...
    print(f"=== UMC Experiment v15a: Big Decoder Probe ===")
    ...
    STRIDE = 4
    BATCH_SIZE = 12
    ACCUM_STEPS = 6  # Effective batch = 72
    RVQ_LEVELS = 8
    PATCH_SIZE = 16
    EPOCHS = 80
    ...
    config = UMCConfig(
        ...
        learning_rate=3e-4,
        # BIGGER decoder (encoder stays 4 layers)
        d_model=256,
        n_heads=8,
        n_encoder_layers=4,
        n_decoder_layers=6,
        ...
        # Same VQ as v14b-A (proven healthy)
        vq_type="vq",
        vq_dim=16,
        vq_top_n_codes=16,
    ...
    print(f"  Encoder layers: {config.n_encoder_layers}, Decoder layers: {config.n_decoder_layers}")
    print(f"  VQ: {config.vq_top_n_codes} top x {config.vq_bottom_n_codes} x {RVQ_LEVELS} RVQ, dim={config.vq_dim}")
    print(f"  Storage: {1 + n_patches * RVQ_LEVELS} bytes/window")

    print(f"\n  Training v15a (big decoder probe, {EPOCHS} epochs)...")
    if device.type == "cuda":
        torch.cuda.empty_cache()
        gc.collect()
    ...
    try:
        best_val = train_hvqvae(
            ...
            epochs=EPOCHS, save_name="v15a", accum_steps=ACCUM_STEPS,
        )
    except torch.cuda.OutOfMemoryError:
        print("  FATAL OOM during v15a training")
        torch.cuda.empty_cache()
        gc.collect()
        state_path = "results/v15a_best_state.pt"
        if os.path.exists(state_path):
            best_state = torch.load(state_path, weights_only=False)
            encoder.load_state_dict(best_state['encoder'])
            decoder.load_state_dict(best_state['decoder'])
    ...
    rmse_pct, entropy_bytes = evaluate_model(
        ...
        batch_size=BATCH_SIZE,
        label="v15a: Big Decoder (d=256, 6 layers)",
    )

    print(f"\n{'='*60}")
    print(f"  v15a RESULT: {rmse_pct:.4f}% RMSE, {5120/entropy_bytes:.1f}x entropy compression")
