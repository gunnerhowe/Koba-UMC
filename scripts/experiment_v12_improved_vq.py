#!/usr/bin/env python3
"""Experiment v12: Improved VQ utilization & compression.

Targeted fixes for v11's three problems:
1. Low codebook utilization (19-25%) -> lower EMA decay, higher dead code threshold
2. Top VQ collapse (2/16 codes) -> shrink to 4 codes
3. Volume RMSE 32.85% -> higher volume weight (5x)

Config changes vs v11:
  - Top VQ: 16 -> 4 codes (only 2 were used)
  - Bottom VQ: 256 -> 128 codes (better entropy efficiency)
  - EMA decay: 0.99 -> 0.95 (faster codebook adaptation)
  - Dead code threshold: 2 -> 5 (more aggressive reset)
  - Commitment weight: 0.1 -> 0.25 (push encoder to codebooks)
  - Volume weight: 3x -> 5x (fix 32% volume RMSE)
  - Dropout: 0.2 -> 0.1 (v11 was underfitting)
  - 100 epochs (quick iteration)

Storage budget:
  Raw: 1 (top) + 16x8 (bottom) = 129 bytes -> 39.7x (naive)
  With entropy coding (128 codes, better util): ~85-95 bytes -> 54-60x
"""
...
def train_hvqvae(encoder, decoder, train_loader, val_loader, config, device,
                 epochs=100, save_name="v12", accum_steps=4):
    """Training loop with targeted improvements for codebook utilization."""
    encoder.to(device)
    decoder.to(device)
    encoder._use_grad_checkpoint = True
    decoder._use_grad_checkpoint = True

    base_lr = 5e-4  # Higher LR for faster convergence in 100 epochs
    ...
    # Feature weights: O, H, L, C(2x), V(5x) -- volume needs way more emphasis
    ...
    patience_counter = 0
    patience = 25  # Shorter patience for 100-epoch run
    warmup_epochs = 5
    ...
    for epoch in range(epochs):
        ...
        aux_weight = min(0.1, 0.1 * epoch / 20.0)  # Ramp up faster (20 vs 30 epochs)
...
def main():
    ...
    print(f"=== UMC Experiment v12: Improved VQ Utilization ===")
    ...
    config = UMCConfig(
        ...
        transformer_dropout=0.1,           # v11: 0.2 -- was underfitting not overfitting
        # VQ -- targeted improvements
        vq_dim=64,
        vq_top_n_codes=4,                  # v11: 16 -- only 2/16 used
        vq_bottom_n_codes=128,             # v11: 256 -- less wasted entropy capacity
        vq_bottom_n_levels=RVQ_LEVELS,
        vq_commitment_weight=0.25,         # v11: 0.1 -- push encoder harder to codebooks
        vq_ema_decay=0.95,                 # v11: 0.99 -- faster codebook adaptation
        vq_dead_code_threshold=5,          # v11: 2 -- more aggressive dead code reset
        # Loss
        beta_start=0.0, beta_end=0.0,
        sparsity_weight=0.0, smoothness_weight=0.0,
        multiscale_weight=0.1, spectral_weight=0.1,
        close_weight=2.0, volume_weight=5.0,  # v11: volume=3.0
    )
    ...
    print(f"  v12 changes: top={config.vq_top_n_codes} (was 16), bottom={config.vq_bottom_n_codes} (was 256)")
    print(f"  EMA decay={config.vq_ema_decay} (was 0.99), dead_code_thresh={config.vq_dead_code_threshold} (was 2)")
    print(f"  Commitment={config.vq_commitment_weight} (was 0.1), dropout={config.transformer_dropout} (was 0.2)")
    print(f"  Storage: 1 + {n_patches}x{RVQ_LEVELS} = {1 + n_patches * RVQ_LEVELS} bytes/window (raw)")

    # === Train ===
    print(f"\n=== Phase 1: Training HVQ-VAE (v12 Improved VQ) ===")
    if device.type == "cuda":
    ...
    try:
        best_val = train_hvqvae(
            ...
            epochs=EPOCHS, save_name="v12", accum_steps=ACCUM_STEPS,
        )
    except torch.cuda.OutOfMemoryError:
        print("  WARNING: CUDA OOM, recovering best state from disk...")
        torch.cuda.empty_cache()
        gc.collect()
        state_path = "results/v12_best_state.pt"
        if os.path.exists(state_path):
            best_state = torch.load(state_path, weights_only=False)
            encoder.load_state_dict(best_state['encoder'])
            decoder.load_state_dict(best_state['decoder'])
    ...
    print()
    print("=" * 60)
    print(f"  EXPERIMENT v12 SUMMARY (improved VQ utilization)")
    print("=" * 60)
    ...
    print(f"  VQ: {config.vq_top_n_codes} top, {config.vq_bottom_n_codes} x {RVQ_LEVELS} RVQ")
    print(f"  Key changes: top={config.vq_top_n_codes}(was 16), bottom={config.vq_bottom_n_codes}(was 256), "
          f"EMA={config.vq_ema_decay}(was 0.99), commit={config.vq_commitment_weight}(was 0.1)")
    print(f"  Total params: {n_total:,}")
    print(f"  Training time: {t_train:.0f}s ({t_train/60:.1f}m)")
    print(f"  VQ path RMSE: {rmse_vq_pct:.4f}%  (target: <0.1%, v11: 2.74%)")
    ...
    print()
    print(f"  v11 baseline: RMSE=2.74%, compression=43.4x")
    if rmse_vq_pct < 2.74:
        print(f"  v12 vs v11 RMSE: {2.74/rmse_vq_pct:.1f}x better")
    else:
        print(f"  v12 vs v11 RMSE: {rmse_vq_pct/2.74:.1f}x worse")
    print("=" * 60)
