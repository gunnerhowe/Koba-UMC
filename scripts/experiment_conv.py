#!/usr/bin/env python3
"""Experiment v3: 1D-CNN encoder/decoder for temporal pattern learning.
...
The CNN architecture captures temporal patterns (trends, reversals,
volatility clusters) that the MLP missed by flattening the input.
"""
...
def train_autoencoder(encoder, decoder, train_loader, val_loader, config, device, epochs=500):
    ...
    for epoch in range(epochs):
        ...
        # Noise schedule: start high, decay
        noise_std = max(0.01, 0.1 * (1 - epoch / epochs))
        ...
        for batch in train_loader:
            ...
            # Add Gaussian noise as data augmentation (denoising autoencoder)
            x_noisy = x + noise_std * torch.randn_like(x)

            enc = encoder.encode(x_noisy)
            x_hat = decoder.decode(enc.z, enc.chart_id)

            # Reconstruct the CLEAN target (not noisy input)
            # Weighted MSE: close price gets 2x weight
            ...
            diff = (x - x_hat) ** 2  # compare to clean x, not x_noisy
...
def main():
    ...
    print(f"=== UMC Experiment v3: 1D-CNN Autoencoder + PCA ===")
    ...
    print(f"\n=== Phase 1: 1D-CNN Autoencoder Training ===")
    ...
    for i, feat in enumerate(config.features):
        ...
        feat_pct = feat_rmse / feat_range * 100
    ...
    print(f"\n  Reconstruction (PCA {target_dims} dims):")
    ...
    n_anomalies = (anomaly_scores > 0.5).sum()
    print(f"  Anomalies detected (threshold=0.5): {n_anomalies} / {len(z_test)}")
    ...
    # Warmup
    with torch.no_grad():
        dummy = torch.randn(1, config.window_size, config.n_features, device=device)
    ...
    try:
        ...
        print(f"  Loaded: {mnf.header.n_samples} samples, {mnf.header.latent_dim} dims")
        mnf_roundtrip_ok = mnf.header.n_samples == len(z_reduced_test)
    finally:
        os.unlink(mnf_path)

    # === Final Summary ===
    print(f"\n{'='*70}")
    print(f"  UMC Experiment v3: 1D-CNN Results")
    print(f"{'='*70}")
    print(f"{'METRIC':<38} {'VALUE':>15} {'TARGET':>8} {'':>6}")
    print(f"{'-'*70}")
    print(f"{'Architecture':<38} {'Conv1D':>15}")
    print(f"{'Model params':<38} {n_params:>15,}")
    print(f"{'Training time':<38} {t_train:>14.1f}s")
    print(f"{'Intrinsic dims (PCA 99%)':<38} {target_dims:>15}")
    print(f"{'-'*70}")
    print(f"{'RMSE % (full dims)':<38} {rmse_full_pct:>14.4f}% {'<0.1%':>8} {'PASS' if rmse_full_pct < 0.1 else 'MISS':>6}")
    print(f"{'RMSE % (PCA reduced)':<38} {rmse_reduced_pct:>14.4f}% {'<0.1%':>8} {'PASS' if rmse_reduced_pct < 0.1 else 'MISS':>6}")
    print(f"{'Per-sample compression':<38} {ratio_per_sample:>14.1f}x {'>50x':>8} {'PASS' if ratio_per_sample > 50 else 'MISS':>6}")
    print(f"{'Full dataset ratio':<38} {ratio_full:>14.1f}x")
    print(f"{'System ratio (+ decoder)':<38} {ratio_system:>14.1f}x")
    print(f"{'gzip (binary) baseline':<38} {gzip_ratio:>14.1f}x")
    print(f"{'Encode (candles/s)':<38} {enc_throughput:>15,.0f} {'>10K':>8} {'PASS' if enc_throughput > 10000 else 'MISS':>6}")
    print(f"{'Decode (candles/s)':<38} {dec_throughput:>15,.0f} {'>50K':>8} {'PASS' if dec_throughput > 50000 else 'MISS':>6}")
    print(f"{'NN search speedup':<38} {speedup:>14.1f}x {'>100x':>8} {'PASS' if speedup > 100 else 'MISS':>6}")
    print(f"{'Anomalies detected':<38} {n_anomalies:>15}")
    print(f"{'MNF roundtrip':<38} {'OK' if mnf_roundtrip_ok else 'FAIL':>15}")
    print(f"{'='*70}")
    ...
    with open("results/experiment_conv.json", "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nSaved to results/experiment_conv.json")
