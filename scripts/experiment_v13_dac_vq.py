#!/usr/bin/env python3
"""Experiment v13: DAC-style VQ for codebook utilization breakthrough.

Root cause of v11/v12 failures: standard VQ in high dimensions suffers from
the "curse of dimensionality" — distance metrics become meaningless in 64D,
causing codebook collapse (only 10-25% utilization).

DAC (Descript Audio Codec) solved this with three key techniques:
  1. codebook_dim=8:     Project to 8D before nearest-neighbor lookup.
                         Low-dimensional space makes L2 distance meaningful.
  2. use_cosine_sim:     L2-normalize codebook vectors. Eliminates magnitude
                         bias that causes some codes to dominate.
  3. rotation_trick:     Better straight-through estimator. Instead of
                         copying gradients through the VQ bottleneck, rotates
                         the gradient direction to be more informative.

Expected impact:
  - Codebook utilization: 10-25% → 60-90%
  - With better utilization, each RVQ level captures more error
  - 8 RVQ levels × better codes → much lower residual error

Config same as v12 except DAC-style VQ params:
  - codebook_dim=8 (NEW)
  - use_cosine_sim=True (NEW)
  - rotation_trick=True (NEW)
  - vq_ema_decay=0.99 (back to standard — cosine_sim fixes the adaptation issue)
  - 150 epochs (longer to let new VQ converge)
"""
...
def train_hvqvae(encoder, decoder, train_loader, val_loader, config, device,
                 epochs=150, save_name="v13", accum_steps=4):
    """Training loop with DAC-style VQ."""
    encoder.to(device)
    decoder.to(device)
...
def main():
    ...
    print(f"=== UMC Experiment v13: DAC-style VQ ===")
    ...
    config = UMCConfig(
        ...
        # VQ -- DAC-style improvements
        vq_dim=64,
        vq_top_n_codes=4,
        vq_bottom_n_codes=128,
        vq_bottom_n_levels=RVQ_LEVELS,
        vq_commitment_weight=0.25,
        vq_ema_decay=0.99,                 # Back to standard (cosine_sim fixes adaptation)
        vq_dead_code_threshold=2,
        vq_codebook_dim=8,                 # DAC: factorized 8D codebook
        vq_use_cosine_sim=True,            # DAC: L2-normalized codebook
        vq_rotation_trick=True,            # DAC: rotation-based STE
        # Loss
    ...
    print(f"  DAC-style: codebook_dim={config.vq_codebook_dim}, "
          f"cosine_sim={config.vq_use_cosine_sim}, rotation_trick={config.vq_rotation_trick}")
    print(f"  Storage: 1 + {n_patches}x{RVQ_LEVELS} = {1 + n_patches * RVQ_LEVELS} bytes/window (raw)")

    # === Train ===
    print(f"\n=== Phase 1: Training HVQ-VAE (v13 DAC-style VQ) ===")
    if device.type == "cuda":
    ...
    try:
        best_val = train_hvqvae(
            ...
            epochs=EPOCHS, save_name="v13", accum_steps=ACCUM_STEPS,
        )
    except torch.cuda.OutOfMemoryError:
        print("  WARNING: CUDA OOM, recovering best state from disk...")
        torch.cuda.empty_cache()
        gc.collect()
        state_path = "results/v13_best_state.pt"
        if os.path.exists(state_path):
            best_state = torch.load(state_path, weights_only=False)
            encoder.load_state_dict(best_state['encoder'])
            decoder.load_state_dict(best_state['decoder'])
    ...
    print()
    print("=" * 60)
    print(f"  EXPERIMENT v13 SUMMARY (DAC-style VQ)")
    print("=" * 60)
    ...
    print(f"  VQ path RMSE: {rmse_vq_pct:.4f}%  (target: <0.1%, v12 baseline: ~3%)")
    ...
    print()
    print(f"  Key metric: Codebook utilization")
    top_util = encoder.top_perplexity / config.vq_top_n_codes * 100
    avg_bot_util = encoder.bottom_perplexity / config.vq_bottom_n_codes * 100
    print(f"    Top: {top_util:.0f}% (v12 was ~50%)")
    print(f"    Bottom avg: {avg_bot_util:.0f}% (v12 was ~10-25%)")
    if avg_bot_util > 50:
        print(f"    DAC-style VQ WORKING -- utilization breakthrough!")
    elif avg_bot_util > 25:
        print(f"    Partial improvement -- consider FSQ fallback")
    else:
        print(f"    Still collapsing -- switch to FSQ immediately")
    print("=" * 60)
