#!/usr/bin/env python3
"""Experiment v6: Wide bottleneck + no pool cap for near-lossless reconstruction.

Root cause analysis of v5 (13.5% RMSE):
  - AdaptiveAvgPool1d(16) on compressed_len=32 HALVES temporal info
  - FC bottleneck: 4096 -> 512 is 8x compression at the bottleneck alone
  - Total FC compression: 8192 -> 512 -> 64 = 128x (way too tight)

v6 changes:
  - pool_size_cap=0 (no cap): preserves all 32 temporal positions
  - bottleneck_dim=1024: wider FC bottleneck for information flow
  - latent_dim=128: 2x more latent capacity (PCA compresses afterward)
  - bottleneck_dropout=0.1: less info destroyed during training
  - stride=2: 2x more training windows to support larger model
  - Very low noise: maximise reconstruction accuracy
"""
...
def train_autoencoder(encoder, decoder, train_loader, val_loader, config, device, epochs=1000):
    """Phase 1: Wide-bottleneck autoencoder training."""
    encoder.to(device)
    decoder.to(device)
    ...
    for epoch in range(epochs):
        ...
        # Very gentle noise — decays to near-zero
        noise_std = max(0.001, 0.01 * (1 - epoch / epochs))
...
def main():
    ...
    print(f"=== UMC Experiment v6: Wide Bottleneck + No Pool Cap ===")
    ...
    LATENT_DIM = 128
    BOTTLENECK = 1024
    STRIDE = 2  # More data to support larger model
    ...
    config = UMCConfig(
        ...
        # Wide bottleneck — the key change
        bottleneck_dim=BOTTLENECK,
        pool_size_cap=0,  # No cap: use full compressed_len (32 for ws=256)
        bottleneck_dropout=0.1,  # Less info loss
    ...
    # === Phase 1: Wide-Bottleneck Conv Autoencoder ===
    print(f"\n=== Phase 1: Wide-Bottleneck Conv AE (bottleneck={BOTTLENECK}, pool_cap=0, latent={LATENT_DIM}) ===")
    ...
    print(f"  Params/window ratio: {n_params / n_train:.0f}")

    # Show architecture details
    n_down = len(encoder.down_blocks)
    compressed = config.window_size // (2 ** n_down)
    flat_dim = 256 * compressed if config.window_size > 64 else 128 * compressed
    print(f"  Architecture: {n_down} down blocks -> compressed_len={compressed} -> flat={flat_dim}")
    print(f"  Info path: {flat_dim} -> {BOTTLENECK} -> {LATENT_DIM} (compression: {flat_dim/LATENT_DIM:.1f}x)")

    t0 = time.perf_counter()
    best_val = train_autoencoder(encoder, decoder, train_loader, val_loader, config, device, epochs=1000)
    ...
    # Decode in normalized space (full latent dims, no PCA)
    decoder.eval()
    decoder.to(device)
    with torch.no_grad():
    ...
    print(f"  Original-scale RMSE (full dims): {rmse_original:.6f} ({rmse_original_pct:.4f}% of range)")
    ...
    # === Summary ===
    print(f"\n{'='*76}")
    print(f"  UMC Experiment v6: Wide Bottleneck Results")
    print(f"  Window: {WINDOW_SIZE} candles x {config.n_features} features = {ambient_dim} ambient dims")
    print(f"  Bottleneck: {BOTTLENECK}, Pool cap: OFF, Latent: {LATENT_DIM}")
    print(f"  Per-window normalization: ENABLED")
    print(f"{'='*76}")
    print(f"{'METRIC':<44} {'VALUE':>15} {'TARGET':>8} {'':>6}")
    print(f"{'-'*76}")
    print(f"{'Architecture':<44} {'Conv1D-wide':>15}")
    ...
    print(f"    v4 (no norm, tight bottleneck):  17.3% RMSE, 66.5x compression")
    print(f"    v5 (per-win norm, tight bot.):   13.5% RMSE, 35.8x compression")
    print(f"    v6 (per-win norm, wide bot.):    {rmse_original_pct:.4f}% RMSE, {ratio_per_sample:.1f}x compression")

    # Save
    Path("results").mkdir(exist_ok=True)
    results = {
        "experiment": "v6_wide_bottleneck",
        "interval": "1h",
        "window_size": WINDOW_SIZE,
        "ambient_dim": ambient_dim,
        "architecture": "conv1d_wide",
        "bottleneck_dim": BOTTLENECK,
        "pool_size_cap": 0,
        "per_window_normalize": True,
        "n_params": n_params,
    ...
    with open("results/experiment_v6.json", "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nSaved to results/experiment_v6.json")
