#!/usr/bin/env python3
"""Experiment v14: FSQ (Finite Scalar Quantization) — eliminate codebook collapse.

Every experiment v2-v13 suffers from VQ codebook collapse:
  - Standard VQ (v2-v9):  3-8% RMSE, 10-25% codebook utilization
  - DAC-style VQ (v13):   9.5% RMSE, 11-12% utilization (WORSE!)

Root cause: learned codebooks collapse regardless of tricks (EMA, cosine_sim,
rotation trick, dead code revival). The encoder routes everything through a few
codes, and the decoder compensates.

FSQ solves this by eliminating learned codebooks entirely:
  - Each dimension is independently quantized to L fixed levels
  - Total implicit codes = product of levels (e.g. [8,8,4] = 256)
  - No codebook vectors to collapse — all grid points are always available
  - No commitment loss, no EMA, no dead code problem
  - Used in production at Google (MagViT2), very stable training

Config:
  - Top: FSQ([8,8]) = 64 implicit codes (vs 4 learned codes in v13)
  - Bottom: ResidualFSQ([8,8,4]) = 256 codes/level, 8 levels (vs 128 learned)
  - Storage: 1 + 16×8 = 129 bytes/window = 39.7x raw compression
  - With entropy coding: ~46-53x (target >50x)
  - Architecture unchanged: d=128, patch=16, 4+4 transformer
"""
...
def train_hvqvae(encoder, decoder, train_loader, val_loader, config, device,
                 epochs=150, save_name="v14", accum_steps=4):
    """Training loop for FSQ model (no VQ commitment loss)."""
    encoder.to(device)
    decoder.to(device)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with torch.amp.autocast('cuda', enabled=use_amp):
                ...
                # FSQ has no VQ commitment loss, but encoder.vq_loss is 0.0 anyway
...
def main():
    ...
    print(f"=== UMC Experiment v14: FSQ (Finite Scalar Quantization) ===")
    ...
    FSQ_BOTTOM_LEVELS = (8, 8, 4)   # 256 implicit codes per RVQ level
    ...
    top_n_codes = int(np.prod(FSQ_TOP_LEVELS))
    bottom_n_codes = int(np.prod(FSQ_BOTTOM_LEVELS))
    ...
    print(f"  FSQ Top: levels={list(FSQ_TOP_LEVELS)} = {top_n_codes} codes")
    print(f"  FSQ Bottom: levels={list(FSQ_BOTTOM_LEVELS)} = {bottom_n_codes} codes x {RVQ_LEVELS} levels")
    print(f"  Storage: 1 + {n_patches}x{RVQ_LEVELS} = {1 + n_patches * RVQ_LEVELS} bytes/window (raw)")
    print(f"  Key advantage: NO codebook collapse possible with FSQ")

    # === Train ===
    print(f"\n=== Phase 1: Training HVQ-VAE (v14 FSQ) ===")
    if device.type == "cuda":
    ...
    try:
        best_val = train_hvqvae(
            ...
            epochs=EPOCHS, save_name="v14", accum_steps=ACCUM_STEPS,
        )
    except torch.cuda.OutOfMemoryError:
        print("  WARNING: CUDA OOM, recovering best state from disk...")
        torch.cuda.empty_cache()
        gc.collect()
        state_path = "results/v14_best_state.pt"
        if os.path.exists(state_path):
            best_state = torch.load(state_path, weights_only=False)
            encoder.load_state_dict(best_state['encoder'])
            decoder.load_state_dict(best_state['decoder'])
    ...
    print(f"\n  Codebook utilization (FSQ implicit codes):")
    print(f"    Top perplexity: {encoder.top_perplexity:.1f} / {top_n_codes} "
          f"({encoder.top_perplexity/top_n_codes*100:.0f}%)")
    for i, p in enumerate(encoder.per_level_perplexity):
        print(f"    Bottom level {i+1}: {p:.1f} / {bottom_n_codes} "
              f"({p/bottom_n_codes*100:.0f}%)")
    ...
    top_counts = np.bincount(top_indices.ravel(), minlength=top_n_codes)
    ...
    total_bits += top_bits
    print(f"  Top FSQ: {top_entropy:.2f} bits/index (max: {np.log2(top_n_codes):.1f})")
    ...
    for lvl in range(RVQ_LEVELS):
        if all_bottom_indices[lvl]:
            ...
            bot_counts = np.bincount(bot_idx.ravel(), minlength=bottom_n_codes)
            ...
            print(f"  Bottom level {lvl+1}: {bot_entropy:.2f} bits/index "
                  f"(max: {np.log2(bottom_n_codes):.1f})")
    ...
    print(f"  VQ ({RVQ_LEVELS}-lvl ResidualFSQ): {raw_bytes} bytes -> {raw_size/raw_bytes:.1f}x")
    pca_bytes = td * 2
    ...
    print()
    print("=" * 60)
    print(f"  EXPERIMENT v14 SUMMARY (FSQ)")
    print("=" * 60)
    ...
    print(f"  FSQ Top: {list(FSQ_TOP_LEVELS)} = {top_n_codes} codes")
    print(f"  FSQ Bottom: {list(FSQ_BOTTOM_LEVELS)} = {bottom_n_codes} codes x {RVQ_LEVELS} levels")
    print(f"  Total params: {n_total:,}")
    ...
    print()
    top_util = encoder.top_perplexity / top_n_codes * 100
    avg_bot_util = encoder.bottom_perplexity / bottom_n_codes * 100
    print(f"  Codebook utilization:")
    print(f"    Top: {encoder.top_perplexity:.1f}/{top_n_codes} ({top_util:.0f}%)")
    print(f"    Bottom avg: {encoder.bottom_perplexity:.1f}/{bottom_n_codes} ({avg_bot_util:.0f}%)")
    if avg_bot_util > 30:
        print(f"    FSQ WORKING — no collapse!")
    else:
        print(f"    Low utilization — FSQ levels may need adjustment")
    print("=" * 60)
