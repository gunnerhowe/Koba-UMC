#!/usr/bin/env python3
"""Experiment v15c: Combined bigger decoder + wider VQ.

Hypothesis: BOTH decoder capacity AND VQ width are bottlenecks.
            The 3% wall needs more representation capacity at every stage.

Changes from v14b-A (2.95% RMSE baseline):
  - vq_dim: 16 -> 24  (1.5x representation)
  - d_model: 128 -> 192  (1.5x decoder width)
  - n_decoder_layers: 4 -> 6  (50% more decoder depth)
  - d_ff: 512 -> 768  (1.5x FFN)
  - vq_bottom_n_codes: 128 -> 256  (denser codebook for 24D)
  - codebook_dim: 12  (factorized codebook in half-dim)
  - batch_size: 12 + accum_steps=6 for VRAM safety

Storage: same 129 bytes/window. All changes are in model capacity, not byte budget.
"""
...
def train_hvqvae(encoder, decoder, train_loader, val_loader, config, device,
                 epochs=80, save_name="v15c", accum_steps=6):
    encoder.to(device)
    decoder.to(device)
...
def main():
    ...
    print(f"=== UMC Experiment v15c: Combined Big Decoder + Wide VQ ===")
    ...
    config = UMCConfig(
        ...
        vq_type="vq",
        vq_dim=24,                   # Middle ground
        vq_codebook_dim=12,          # Factorized
        vq_top_n_codes=16,
    ...
    print(f"\n  Training v15c (combined probe, {EPOCHS} epochs)...")
    if device.type == "cuda":
        torch.cuda.empty_cache()
        gc.collect()
    ...
    try:
        best_val = train_hvqvae(
            ...
            epochs=EPOCHS, save_name="v15c", accum_steps=ACCUM_STEPS,
        )
    except torch.cuda.OutOfMemoryError:
        print("  FATAL OOM during v15c training")
        torch.cuda.empty_cache()
        gc.collect()
        state_path = "results/v15c_best_state.pt"
        if os.path.exists(state_path):
            best_state = torch.load(state_path, weights_only=False)
            encoder.load_state_dict(best_state['encoder'])
            decoder.load_state_dict(best_state['decoder'])
    ...
    rmse_pct, entropy_bytes = evaluate_model(
        ...
        batch_size=BATCH_SIZE,
        label="v15c: Combined (d=192, vq_dim=24, 6 dec layers)",
    )

    print(f"\n{'='*60}")
    print(f"  v15c RESULT: {rmse_pct:.4f}% RMSE, {5120/entropy_bytes:.1f}x entropy compression")
